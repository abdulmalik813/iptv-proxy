...
export async function POST(req: NextRequest) {
  try {
    ...
    if (Number(countRes.rows[0]?.count || 0) > 0) {
      return NextResponse.json({ success: false, error: 'Initial administrator setup is already complete.' }, { status: 409 });
    }
    ...
    try {
      await db.batch([
        {
          ...
          args: [userId, username, passwordHash, now, now],
        },
        {
      ...
      if (Number(check.rows[0]?.count || 0) > 0) {
        return NextResponse.json({ success: false, error: 'Administrator setup was completed by another request.' }, { status: 409 });
      }
      throw error;
    }

    const token = await createSessionToken({ id: userId, username, session_version: 1 });
