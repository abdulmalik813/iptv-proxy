...
export async function POST(req: NextRequest) {
  try {
    ...
    const key = loginRateLimitKey(clientIp(req), username);
    ...
    const userRes = await getDb().execute({
      sql: 'SELECT id, username, password_hash, session_version FROM users WHERE username = ?',
      args: [username],
    });
    const user = userRes.rows[0];
    const passwordMatch = user ? await verifyPassword(password, String(user.password_hash)) : false;
    if (!user || !passwordMatch) {
      recordLoginFailure(key);
      await LogService.warn('auth', 'login_failure', `Failed login attempt for username "${username}".`, { ip: clientIp(req) });
