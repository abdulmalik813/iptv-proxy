...
const createProviderSchema = z.object({
  ...
  local_username: z.string().trim().min(1, 'Local username is required').max(512),
  local_password: z.string().min(1, 'Local password is required').max(4096),
...
export async function POST(req: NextRequest) {
  try {
    ...
    return NextResponse.json({ success: true, data: await ProviderService.createProvider(parsed.data) }, { status: 201 });
  } catch (error) {
